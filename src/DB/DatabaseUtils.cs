                ...
                else
                {
                    using (JsonDocument? jsonConfig = Utils.LoadJsonOnMainThread(dbPath)!)
                    {
                        if (jsonConfig != null)
                        {
                            ...
                            {
                                return
                                    $"Server={host};Database={database};User ID={username};Password={password};Port={port};SslMode={sslMode}";
                            }
            ...
            switch (dbType)
            {
                case DatabaseType.MySQL:
                    query = @"CREATE TABLE IF NOT EXISTS PlayerStageTimes (
                        ...
                        Mode VARCHAR(24) NOT NULL DEFAULT 'None',
                ...
                using (var connection = await OpenConnectionAsync())
                {
                    ...
                    if (row.Read())
                    {
                        ...
                        using (upsertCommand)
                        {
                            ...
                            if (enableReplays)
                            {
                                var capturedStyle = playerTimers[slot].currentStyle;
                                var capturedMode = playerTimers[slot].Mode;
                                if (onlySRReplay && (prevSRTimerTicks == 0 || prevSRTimerTicks > timerTicks))
                                {
                                    if (dBtimerTicks == 0 || timerTicks < dBtimerTicks)
                                    {
                                        if (useBinaryReplays)
                                            _ = Task.Run(async () => await DumpReplayToBinary(player!, steamId, slot, bonusX, capturedStyle, capturedMode));
                                        else
                                            _ = Task.Run(async () => await DumpReplayToJson(player!, steamId, slot, bonusX, capturedStyle, capturedMode));
                                    }
                                }
                            ...
                            var capturedModeForGlobal = playerTimers.TryGetValue(slot, out var timerForGlobal) ? timerForGlobal.Mode : mode;
                            Server.NextFrame(async () =>
                            {
                                 if (globalDisabled)
                                    return;

                                 if (!playerCache.PlayerID.TryGetValue(slot, out int playerId))
                                 {
                                    Utils.LogDebug($"Skipping global record submit for {playerName}: missing PlayerID for slot {slot}");
                                    return;
                                 }
                                 
                                 DateTimeOffset timeCreated = DateTimeOffset.UtcNow;

                                 if (capturedModeForGlobal == "Custom")
                                    return;

                                 //first lets see if the new record beats global pb
                                 var beatGlobalPB = false;
                                 var prevPBTime = await GetPreviousPlayerRecordFromGlobal(playerId, capturedModeForGlobal,
                    ...
                    else
                    {
                        ...
                        using (upsertCommand)
                        {
                            ...
                            if (enableReplays)
                            {
                                var capturedStyle2 = playerTimers[slot].currentStyle;
                                var capturedMode2 = playerTimers[slot].Mode;
                                if (onlySRReplay && (prevSRTimerTicks == 0 || prevSRTimerTicks > timerTicks))
                                {
                                    if (dBtimerTicks == 0 || timerTicks < dBtimerTicks)
                                    {
                                        if (useBinaryReplays)
                                            _ = Task.Run(async () => await DumpReplayToBinary(player!, steamId, slot, bonusX, capturedStyle2, capturedMode2));
                                        else
                                            _ = Task.Run(async () => await DumpReplayToJson(player!, steamId, slot, bonusX, capturedStyle2, capturedMode2));
                                    }
                                }
                                else if (!onlySRReplay)
                                {
                            ...
                            var capturedModeForGlobal2 = playerTimers.TryGetValue(slot, out var timerForGlobal2) ? timerForGlobal2.Mode : mode;
                            Server.NextFrame(async () =>
                            {
                                if (globalDisabled)
                                    return;
                                ...
                                if (capturedModeForGlobal2 == "Custom")
                                    return;

                                //first lets see if the new record beats global pb
                                var beatGlobalPB = false;
                                var prevPBTime = await GetPreviousPlayerRecordFromGlobal(playerId, capturedModeForGlobal2,
                                    GetNamedStyle(style), bonusX);
                                if (prevPBTime > Utils.TicksToDecimal(timerTicks) || prevPBTime == 0)
                                    beatGlobalPB = true;
                ...
                using (var connection = await OpenConnectionAsync())
                {
                    ...
                    if (row.Read())
                    {
                        ...
                        using (upsertCommand)
                        {
                            ...
                            upsertCommand!.AddParameterWithValue("@Style", style);
                            var stageMode1 = playerTimers.TryGetValue(slot, out var stTimer1) ? stTimer1.Mode : "None";
                            upsertCommand!.AddParameterWithValue("@Mode", stageMode1);
                            ...
                            var prevSRID = await GetStageRecordSteamIDFromDatabase(stage, style, stageMode1);
                    ...
                    else
                    {
                        ...
                        using (upsertCommand)
                        {
                            ...
                            upsertCommand!.AddParameterWithValue("@Velocity", velocity);
                            upsertCommand!.AddParameterWithValue("@Style", style);
                            var stageMode2 = playerTimers.TryGetValue(slot, out var stTimer2) ? stTimer2.Mode : "None";
                            upsertCommand!.AddParameterWithValue("@Mode", stageMode2);
                            var prevSRID = await GetStageRecordSteamIDFromDatabase(stage, style, stageMode2);
                            var prevSR = await GetPreviousPlayerStageRecordFromDatabase(player, prevSRID.Item1,
                                currentMapNamee, stage, prevSRID.Item2, bonusX);
                            await upsertCommand!.ExecuteNonQueryAsync();
                ...
                try
                {
                    ...
                    if (limit != 0)
                    {
                        switch (dbType)
                        {
                            case DatabaseType.MySQL:
                                selectQuery =
                                    $@"SELECT SteamID, PlayerName, TimerTicks, TimesFinished FROM PlayerRecords WHERE MapName = @MapName AND Style = @Style AND Mode = @Mode ORDER BY TimerTicks ASC LIMIT {limit}";
                                selectCommand = new MySqlCommand(selectQuery, (MySqlConnection)connection);
                                break;
                            case DatabaseType.PostgreSQL:
                                selectQuery =
                    ...
                    else
                    {
                        switch (dbType)
                        {
                            case DatabaseType.MySQL:
                                selectQuery =
                                    @"SELECT SteamID, PlayerName, TimerTicks, TimesFinished FROM PlayerRecords WHERE MapName = @MapName AND Style = @Style AND Mode = @Mode";
                                selectCommand = new MySqlCommand(selectQuery, (MySqlConnection)connection);
                                break;
                            case DatabaseType.PostgreSQL:
                                selectQuery =
