        ...
        public HookResult TriggerMultiple_OnStartTouch(CEntityIOOutput output, string name, CEntityInstance activator, CEntityInstance caller, CVariant value, float delay)
        {
            try
            {
                ...
                if (useStageTriggers == true && stageTriggers.ContainsKey(callerHandle) && playerTimers[slot].IsTimerBlocked == false && playerTimers[slot].IsTimerRunning == true)
                {
                    ...
                    {
                        var capturedStyleStage = playerTimers[slot].currentStyle;
                        var capturedModeStage = playerTimers[slot].Mode;
                        _ = Task.Run(async () => await HandlePlayerStageTimes(player, callerHandle, slot, steamID, playerName, capturedStyleStage, capturedModeStage));
                        return HookResult.Continue;
                    }
                }
                ...
                {
                    var capturedStyleCp = playerTimers[slot].currentStyle;
                    var capturedModeCp = playerTimers[slot].Mode;
                    _ = Task.Run(async () => await HandlePlayerCheckpointTimes(player, callerHandle, slot, steamID, playerName, capturedStyleCp, capturedModeCp));
                    return HookResult.Continue;
                }
                ...
                {
                    var capturedStyleBcp = playerTimers[slot].currentStyle;
                    var capturedModeBcp = playerTimers[slot].Mode;
                    _ = Task.Run(async () => await HandlePlayerBonusCheckpointTimes(player, callerHandle, slot, steamID, playerName, capturedStyleBcp, capturedModeBcp));
                    return HookResult.Continue;
                }
